...
TEST(TestGeomUtils, TestBisector)
{
    ...
    Line2D ans = Line2D::fromCoefficients(std::vector<long double>{0.99, 0.16, -2.61});
    std::cout << ans << '\n';
    std::cout << ans(O) << ' ' << bisector(O) << '\n';
    std::cout << ans(K) << ' ' << bisector(K) << '\n';
    ASSERT_TRUE(almost_eq(fabsl(ans(O)), fabsl(bisector(O)), 0.05));
    ASSERT_TRUE(almost_eq(fabsl(ans(K)), fabsl(bisector(K)), 0.05));
}
